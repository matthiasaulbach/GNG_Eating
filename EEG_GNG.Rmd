...
###### And the ERP data:
ERP_tidy <- readRDS("ERP_tidy_200to290.rds")
...
ERP_tidy <- ERP_tidy %>%
  mutate(BaselineDiff_Cz = 
           (((High_No.1.Balanced.Cz_200to290 + High_No.2.Balanced.Cz_200to290)/2) - ((High_Go.1.Balanced.Cz_200to290 + High_Go.2.Balanced.Cz_200to290)/2)) -
         ...
         BaselineDiff_FC1 = 
           (((High_No.1.Balanced.FC1_200to290 + High_No.2.Balanced.FC1_200to290)/2) - ((High_Go.1.Balanced.FC1_200to290 + High_Go.2.Balanced.FC1_200to290)/2)) -
         ...
         BaselineDiff_FC2 = 
           (((High_No.1.Balanced.FC2_200to290 + High_No.2.Balanced.FC2_200to290)/2) - ((High_Go.1.Balanced.FC2_200to290 + High_Go.2.Balanced.FC2_200to290)/2)) -
         ...
         BaselineDiff_Fz = 
           (((High_No.1.Balanced.Fz_200to290 + High_No.2.Balanced.Fz_200to290)/2) - ((High_Go.1.Balanced.Fz_200to290 + High_Go.2.Balanced.Fz_200to290)/2)) -
...
ERP_long <- ERP_tidy %>%
  select(., - c("LowCalGo_DiffCz_1":"Subject")) %>%
...
Looks like participant 10 is somewhat off but only in the Balanced block is he/she outside the +/- 3 sd range.
...
ggplot(ERP_anova_change, aes(Block, difference.score, color = Cycle)) + 
  geom_boxplot()
...
So we _do_ see the expected main effect of Cycle, also a main effect of food type (with the Low-Calorie Go block leading to _larger_ N2-difference scores) but _no_ interaction.
...
So now we check whether the difference between intake after High-Calorie Go and Low-Calorie Go is related to the N2 difference score (the score from hypothesis 3).
...
Now I want to create change scores for the different kinds of trials themselves (not for the difference as above). This way we can try to figure out more precisely what is happening.
```{r}

# IMPORTANT: these are now only the DOMINANT trials, i.e. those we want to work with!
...
ggplot(ERP_plot_by_trialtype, aes(Cycle, N2, color = Trial)) +
  geom_boxplot()


ERP_plot_by_trialtype_by_Cycle_summary <- ERP_plot_by_trialtype %>% group_by(Trial, Cycle) %>% summarise_at(., "N2", funs(mean, sd))

ggplot(ERP_plot_by_trialtype_by_Cycle_summary, aes(Cycle, mean, color = Trial, group = Trial)) +
  geom_point() + 
  geom_line(stat = "identity") + 
  geom_errorbar(aes(ymin = mean-2*(sd/49), ymax = mean+2*(sd/49)), inherit = TRUE, width = 0.1) +
  ggtitle("Mean N2 amplitude scores for the biased blocks (with SEs)")


# This model tests the effects of trial type and Cycle on the N2 amplitude
fit_bias_by_trial <- lmerTest::lmer(N2 ~ Trial*Cycle + (1|pp),
                                      data=ERP_plot_by_trialtype)
anova_bias_by_trial <- anova(fit_bias_by_trial)
anova_bias_by_trial
summary(fit_bias_by_trial)
# We can see that HighNo trials elicit the strongest N2 response 
# and that overall, Cycle 2 has less pronounced N2 amplitudes than Cycle 1
```
...
fit_change_by_trialtype <- lmerTest::lmer(N2_change ~ HighLow*GoNo + (1|pp),
                                      data=N2_change_df)
anova_change_by_trialtype <- anova(fit_change_by_trialtype)
anova_change_by_trialtype 
...
summary(fit_change_by_trialtype)
...
ggplot(plot_N2_change_df, aes(x = GoNo, y = mean_change, colour = HighLow)) +
  geom_point() + 
  geom_errorbar(aes(ymin = mean_change-2*(sd/49), ymax = mean_change+2*(sd/49)), inherit = TRUE, width = 0.1)
```
...
fit_trainingblocks_by_trialtype <- lmerTest::lmer(N2 ~ HighLow*GoNo + (1|pp),
                                      data=ERP_plot_2by2Trial)
anova_trainingblocks_by_trialtype <- anova(fit_trainingblocks_by_trialtype)
anova_trainingblocks_by_trialtype 
```
...
anova(lmerTest::lmer(N2 ~ HighLow*GoNo*Cycle + (1|pp), data = ERP_plot_2by2Trial))
```

This is looking at Go vs NoGo, High vs Low, and Cycle instead of difference scores (which, of course, leads to nasty three-way interaction terms). Again, there's significant main effects of GoNo and Cycle but no interactions.
All this indicates that N2 becomes less pronounced with training _regardless of trial type_.
...
Balanced_Cycles <- full_withERP %>%
  select(pp,
         HighGo_1_Balanced.allsites:LowNo_2_LowCalGo.allsites) %>%
  gather("Trial_Cycle_Block", "N2", HighGo_1_Balanced.allsites:LowNo_2_LowCalGo.allsites) %>%
...
ggplot(Balanced_Cycles_summary, aes(x = Cycle, y = mean_N2, color = GoNo, group = GoNo)) +
  geom_point() + 
  geom_line(stat = "identity") +
  geom_errorbar(aes(ymin = mean_N2-2*(sd/49), ymax = mean_N2+2*(sd/49)), inherit = TRUE, width = 0.1) +
  facet_wrap(aes(Facets = HighLow))  

anova(lmerTest::lmer(N2 ~ HighLow*GoNo*Cycle + (1|pp), data = Balanced_Cycles))
```

So the effects in the Balanced block are pretty much the same as in the Bias blocks: Main effects of Go vs No and of Cycle with no interactions. This seems to indicate that the training blocks don't do anything that the Balanced block doesn't also do...
